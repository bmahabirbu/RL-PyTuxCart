    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None):
        ...
        if self.k is not None and self.k.config.track == track:
            ...
            if self.k is not None:
                self.k.stop()
                del self.k
            config = pystk.RaceConfig(num_kart=1, laps=1,track=track, difficulty = 2)
        ...
        for t in range(max_frames):
           ...
           #print(arrayTest[t])
            #print(test)
            if(len(arrayTest) == 0):
                aim_point_world = self._point_on_track(kart.distance_down_track+TRACK_OFFSET-10, track)
                arrayTest.append(aim_point_world[0])
                   
            if len(arrayTest) > 1:
                print(arrayTest[t] - arrayTest[t-1])
                if arrayTest[t] == arrayTest[t-1]:
                    test = test + 10
                    
            if test > 1:
                aim_point_world = self._point_on_track(kart.distance_down_track-10, track)
                test = test-2
                #print("Go back, dummy")
            else:
                aim_point_world = self._point_on_track(kart.distance_down_track+TRACK_OFFSET-10, track)
