...
class Planner(torch.nn.Module):
    def __init__(self):
        ...
        # 3 is the dimension of the input bc image is 3 arrays
        # 16 output arrays, every one of the three gets 5, everyone gets 5 kernels, one extra kernel gets assigned
        # 5 is size of the kernel (5x5)
        # 2 is the stride
        # 2 is the padding
        ...
        layers.append(torch.nn.ConvTranspose2d(128,128,5,2,2))
        layers.append(torch.nn.ReLU())
        layers.append(torch.nn.BatchNorm2d(128))
        layers.append(torch.nn.Conv2d(128,64,5,2,2))
        layers.append(torch.nn.ReLU())
        layers.append(torch.nn.BatchNorm2d(64))
        layers.append(torch.nn.Conv2d(64,32,5,2,2))
        layers.append(torch.nn.ReLU())
        layers.append(torch.nn.BatchNorm2d(32))
        layers.append(torch.nn.ConvTranspose2d(32,32,5,2,2))
        layers.append(torch.nn.ReLU())
        layers.append(torch.nn.BatchNorm2d(32))
