...
def control(aim_point, current_vel):
    ...
    elif(current_vel > target_velocity/2 and current_vel < 3*target_velocity/4):
        action.acceleration = 0.97377
        #0.97377
    elif(current_vel > 3*target_velocity/4 and current_vel < target_velocity):
        action.acceleration = -0.140493
        #-0.140493
    elif(current_vel >= target_velocity):
        action.acceleration = -0.94901
        #action.brake = True
        #-0.94901
    #print("Accel",action.acceleration)


    if(abs(aim_point[0]) <= 0.025):
        action.steer = 0
        
    elif(abs(aim_point[0]) >= 0.025 and abs(aim_point[0]) <= 0.1):
